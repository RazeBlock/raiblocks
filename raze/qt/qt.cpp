...
bool raze_qt::eventloop_processor::event (QEvent * event_a)
{
	assert (dynamic_cast<raze_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<raze_qt::eventloop_event *> (event_a)->action ();
	return true;
}

raze_qt::eventloop_event::eventloop_event (std::function<void()> const & action_a) :
QEvent (QEvent::Type::User),
action (action_a)
{
}

raze_qt::self_pane::self_pane (raze_qt::wallet & wallet_a, raze::account const & account_a) :
window (new QWidget),
...
wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("Version %1%.%2%") % RAZE_VERSION_MAJOR % RAZE_VERSION_MINOR).c_str ());
...
void raze_qt::self_pane::refresh_balance ()
{
...
raze_qt::accounts::accounts (raze_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
	...
	QObject::connect (backup_seed, &QPushButton::released, [this]() {
		raze::raw_key seed;
		raze::transaction transaction (this->wallet.wallet_m->store.environment, nullptr, false);
		if (this->wallet.wallet_m->store.valid_password (transaction))
		{
...
void raze_qt::accounts::refresh_wallet_balance ()
{
	raze::transaction transaction (this->wallet.wallet_m->store.environment, nullptr, false);
...
void raze_qt::accounts::refresh ()
{
	model->removeRows (0, model->rowCount ());
	raze::transaction transaction (wallet.wallet_m->store.environment, nullptr, false);
	QBrush brush;
...
raze_qt::import::import (raze_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
raze_qt::history::history (raze::ledger & ledger_a, raze::account const & account_a, raze_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
model (new QStandardItemModel),
view (new QTableView),
...
namespace
{
class short_text_visitor : public raze::block_visitor
{
public:
	short_text_visitor (MDB_txn * transaction_a, raze::ledger & ledger_a) :
	...
	void open_block (raze::open_block const & block_a)
	{
		...
		{
			account = raze::genesis_account;
			amount = raze::genesis_amount;
		}
	}
...
void raze_qt::history::refresh ()
{
	raze::transaction transaction (ledger.store.environment, nullptr, false);
	model->removeRows (0, model->rowCount ());
...
raze_qt::block_viewer::block_viewer (raze_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
	...
	QObject::connect (retrieve, &QPushButton::released, [this]() {
		raze::block_hash hash_l;
		if (!hash_l.decode_hex (hash->text ().toStdString ()))
		{
			raze::transaction transaction (this->wallet.node.store.environment, nullptr, false);
...
void raze_qt::block_viewer::rebroadcast_action (raze::uint256_union const & hash_a)
{
	auto done (true);
	raze::transaction transaction (wallet.node.ledger.store.environment, nullptr, false);
...
raze_qt::account_viewer::account_viewer (raze_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
raze_qt::status::status (raze_qt::wallet & wallet_a) :
wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (raze_qt::status_types::nominal);
	set_text ();
}

void raze_qt::status::erase (raze_qt::status_types status_a)
{
	assert (status_a != raze_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void raze_qt::status::insert (raze_qt::status_types status_a)
{
	assert (status_a != raze_qt::status_types::nominal);
...
std::string raze_qt::status::text ()
{
	...
	{
		raze::transaction transaction (wallet.wallet_m->node.store.environment, nullptr, false);
	...
	switch (*active.begin ())
	{
		case raze_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case raze_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case raze_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case raze_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case raze_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case raze_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case raze_qt::status_types::nominal:
...
std::string raze_qt::status::color ()
{
	assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
active_status (*this)
{
	...
	send_account->setPlaceholderText (raze::zero_key.pub.to_account ().c_str ());
...
void raze_qt::wallet::start ()
{
	std::weak_ptr<raze_qt::wallet> this_w (shared_from_this ());
	...
	QObject::connect (send_blocks_send, &QPushButton::released, [this_w]() {
		if (auto this_l = this_w.lock ())
		{
			...
			{
				raze::uint128_t actual (amount.number () * this_l->rendering_ratio);
				if (actual / this_l->rendering_ratio == amount.number ())
				{
					...
					if (!parse_error)
					{
						...
						if (actual <= balance)
						{
							raze::transaction transaction (this_l->wallet_m->store.environment, nullptr, false);
							if (this_l->wallet_m->store.valid_password (transaction))
							{
								...
								this_l->node.background ([this_w, account_l, actual]() {
									if (auto this_l = this_w.lock ())
									{
										this_l->wallet_m->send_async (this_l->account, account_l, actual, [this_w](std::shared_ptr<raze::block> block_a) {
											if (auto this_l = this_w.lock ())
											{
	...
	node.observers.blocks.add ([this_w](std::shared_ptr<raze::block> block_a, raze::account const & account_a, raze::amount const &) {
		if (auto this_l = this_w.lock ())
		{
	...
	node.observers.account_balance.add ([this_w](raze::account const & account_a, bool is_pending) {
		if (auto this_l = this_w.lock ())
		{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (raze_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (raze_qt::status_types::active);
					}
				}
			}));
		}
	});
	node.observers.endpoint.add ([this_w](raze::endpoint const &) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w]() {
				if (auto this_l = this_w.lock ())
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (raze_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (raze_qt::status_types::synchronizing);
					}
				}
			}));
		}
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working]() {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (raze_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (raze_qt::status_types::working);
					}
				}
			}));
		}
...
void raze_qt::wallet::refresh ()
{
	{
		raze::transaction transaction (wallet_m->store.environment, nullptr, false);
...
void raze_qt::wallet::update_connected ()
{
	if (node.peers.empty ())
	{
		active_status.insert (raze_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (raze_qt::status_types::disconnected);
	}
}

void raze_qt::wallet::empty_password ()
{
...
void raze_qt::wallet::change_rendering_ratio (raze::uint128_t const & rendering_ratio_a)
{
...
void raze_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void raze_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

raze_qt::settings::settings (raze_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
password (new QLineEdit),
...
wallet (wallet_a)
{
	...
	new_representative->setPlaceholderText (raze::zero_key.pub.to_account ().c_str ());
	...
	QObject::connect (change_rep, &QPushButton::released, [this]() {
		...
		if (!representative_l.decode_account (new_representative->text ().toStdString ()))
		{
			...
			if (this->wallet.wallet_m->store.valid_password (transaction))
			{
				...
				{
					raze::transaction transaction_l (this->wallet.wallet_m->store.environment, nullptr, true);
...
void raze_qt::settings::refresh_representative ()
{
	raze::transaction transaction (this->wallet.wallet_m->node.store.environment, nullptr, false);
...
void raze_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void raze_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (raze_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (raze_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (raze_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (raze_qt::status_types::vulnerable);
	}
}

raze_qt::advanced_actions::advanced_actions (raze_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
	ratio_group->addButton (mraze);
	ratio_group->addButton (kraze);
	ratio_group->addButton (raze);
	ratio_group->setId (mraze, 0);
	ratio_group->setId (kraze, 1);
	ratio_group->setId (raze, 2);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (mraze);
	scale_layout->addWidget (kraze);
	scale_layout->addWidget (raze);
	...
	QObject::connect (peers_bootstrap, &QPushButton::released, [this]() {
		...
		auto error (raze::parse_endpoint (bootstrap_line->text ().toStdString (), endpoint));
		if (!error)
		{
...
void raze_qt::advanced_actions::refresh_peers ()
{
...
void raze_qt::advanced_actions::refresh_ledger ()
{
	...
	{
		QList<QStandardItem *> items;
		items.push_back (new QStandardItem (QString (raze::block_hash (i->first.uint256 ()).to_account ().c_str ())));
		raze::account_info info (i->second);
		std::string balance;
		raze::amount (info.balance.number () / wallet.rendering_ratio).encode_dec (balance);
...
raze_qt::block_entry::block_entry (raze_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
block (new QPlainTextEdit),
status (new QLabel),
...
wallet (wallet_a)
{
	...
	QObject::connect (process, &QPushButton::released, [this]() {
		...
		try
		{
			...
			auto block_l (raze::deserialize_block_json (tree));
			if (block_l != nullptr)
			{
...
raze_qt::block_creation::block_creation (raze_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void raze_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void raze_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void raze_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void raze_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void raze_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
			...
			if (!error)
			{
				...
				if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
				{
					...
					if (amount_l.number () <= balance)
					{
						...
						assert (!error);
						raze::send_block send (info.head, destination_l, balance - amount_l.number (), key, account_l, wallet.wallet_m->work_fetch (transaction, account_l, info.head));
						std::string block_l;
...
void raze_qt::block_creation::create_receive ()
{
	raze::block_hash source_l;
	...
	if (!error)
	{
		...
		if (block_l != nullptr)
		{
			auto send_block (dynamic_cast<raze::send_block *> (block_l.get ()));
			if (send_block != nullptr)
			{
				raze::pending_key pending_key (send_block->hashables.destination, source_l);
				...
				if (!wallet.node.store.pending_get (transaction, pending_key, pending))
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							raze::receive_block receive (info.head, source_l, key, pending_key.account, wallet.wallet_m->work_fetch (transaction, pending_key.account, info.head));
							std::string block_l;
...
void raze_qt::block_creation::create_change ()
{
	raze::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		...
		if (!error)
		{
			...
			if (!error)
			{
				...
				if (!error)
				{
					raze::change_block change (info.head, representative_l, key, account_l, wallet.wallet_m->work_fetch (transaction, account_l, info.head));
					std::string block_l;
...
void raze_qt::block_creation::create_open ()
{
	raze::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
				...
				if (send_block != nullptr)
				{
					...
					if (!wallet.node.store.pending_get (transaction, pending_key, pending))
					{
						...
						if (error)
						{
							...
							if (!error)
							{
								raze::open_block open (source_l, representative_l, pending_key.account, key, pending_key.account, wallet.wallet_m->work_fetch (transaction, pending_key.account, pending_key.account));
								std::string block_l;
