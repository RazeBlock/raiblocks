...
public:
	...
	QPushButton * back;
	raze_qt::wallet & wallet;
};
class advanced_actions
{
public:
	advanced_actions (raze_qt::wallet &);
	QWidget * window;
	QVBoxLayout * layout;
...
{
public:
	self_pane (raze_qt::wallet &, raze::account const &);
...
{
public:
	history (raze::ledger &, raze::account const &, raze_qt::wallet &);
	void refresh ();
	QWidget * window;
	QVBoxLayout * layout;
	QStandardItemModel * model;
	...
	QSpinBox * tx_count;
	raze::ledger & ledger;
	raze::account const & account;
	raze_qt::wallet & wallet;
};
class block_viewer
{
public:
	block_viewer (raze_qt::wallet &);
	void rebroadcast_action (raze::uint256_union const &);
	QWidget * window;
	QVBoxLayout * layout;
	QLabel * hash_label;
	QLineEdit * hash;
...
{
public:
	status (raze_qt::wallet &);
	void erase (raze_qt::status_types);
	void insert (raze_qt::status_types);
	void set_text ();
	std::string text ();
	std::string color ();
	std::set<raze_qt::status_types> active;
	raze_qt::wallet & wallet;
};
class wallet : public std::enable_shared_from_this<raze_qt::wallet>
{
public:
	wallet (QApplication &, raze_qt::eventloop_processor &, raze::node &, std::shared_ptr<raze::wallet>, raze::account &);
	void start ();
	void refresh ();
	void update_connected ();
	void empty_password ();
	void change_rendering_ratio (raze::uint128_t const &);
	std::string format_balance (raze::uint128_t const &) const;
	raze::uint128_t rendering_ratio;
	raze::node & node;
	std::shared_ptr<raze::wallet> wallet_m;
	raze::account & account;
	raze_qt::eventloop_processor & processor;
	raze_qt::history history;
	raze_qt::accounts accounts;
	raze_qt::self_pane self;
	raze_qt::settings settings;
	raze_qt::advanced_actions advanced;
	raze_qt::block_creation block_creation;
	raze_qt::block_entry block_entry;
	raze_qt::block_viewer block_viewer;
	raze_qt::account_viewer account_viewer;
	raze_qt::import import;
