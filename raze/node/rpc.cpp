...
raze::rpc_config::rpc_config () :
address (boost::asio::ip::address_v6::loopback ()),
port (raze::rpc::rpc_port),
...
raze::rpc_config::rpc_config (bool enable_control_a) :
address (boost::asio::ip::address_v6::loopback ()),
port (raze::rpc::rpc_port),
...
void raze::rpc_config::serialize_json (boost::property_tree::ptree & tree_a) const
{
...
bool raze::rpc_config::deserialize_json (boost::property_tree::ptree const & tree_a)
{
	auto result (false);
	try
	{
...
raze::rpc::rpc (boost::asio::io_service & service_a, raze::node & node_a, raze::rpc_config const & config_a) :
acceptor (service_a),
config (config_a),
node (node_a)
{
	auto endpoint (raze::tcp_endpoint (config_a.address, config_a.port));
	...
	acceptor.listen ();
	node_a.observers.blocks.add ([this](std::shared_ptr<raze::block> block_a, raze::account const & account_a, raze::amount const &) {
...
void raze::rpc::start ()
{
	auto connection (std::make_shared<raze::rpc_connection> (node, *this));
...
raze::rpc_handler::rpc_handler (raze::node & node_a, raze::rpc & rpc_a, std::string const & body_a, std::function<void(boost::property_tree::ptree const &)> const & response_a) :
body (body_a),
node (node_a),
rpc (rpc_a),
response (response_a)
...
void raze::rpc::observer_action (raze::account const & account_a)
{
	std::shared_ptr<raze::payment_observer> observer;
	{
...
void raze::rpc_handler::account_balance ()
{
...
void raze::rpc_handler::account_block_count ()
{
	std::string account_text (request.get<std::string> ("account"));
	raze::uint256_union account;
	auto error (account.decode_account (account_text));
...
void raze::rpc_handler::account_create ()
{
	if (rpc.config.enable_control)
	{
		...
		if (!error)
		{
			...
			if (existing != node.wallets.items.end ())
			{
				...
				raze::account new_key (existing->second->deterministic_insert (generate_work));
				if (!new_key.is_zero ())
				{
					boost::property_tree::ptree response_l;
...
void raze::rpc_handler::account_get ()
{
...
void raze::rpc_handler::account_info ()
{
	std::string account_text (request.get<std::string> ("account"));
	raze::uint256_union account;
	auto error (account.decode_account (account_text));
	if (!error)
	{
		...
		if (!node.store.account_get (transaction, account, info))
		{
			...
			std::string balance;
			raze::uint128_union (info.balance).encode_dec (balance);
...
void raze::rpc_handler::account_key ()
{
	std::string account_text (request.get<std::string> ("account"));
...
void raze::rpc_handler::account_list ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	raze::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
	if (!error)
	{
		...
		if (existing != node.wallets.items.end ())
		{
			...
			for (auto i (existing->second->store.begin (transaction)), j (existing->second->store.end ()); i != j; ++i)
			{
				...
				entry.put ("", raze::uint256_union (i->first.uint256 ()).to_account ());
...
void raze::rpc_handler::account_move ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
			...
			if (existing != node.wallets.items.end ())
			{
				...
				if (!error)
				{
					...
					if (existing != node.wallets.items.end ())
					{
						...
						std::vector<raze::public_key> accounts;
						for (auto i (accounts_text.begin ()), n (accounts_text.end ()); i != n; ++i)
						{
							raze::public_key account;
...
void raze::rpc_handler::account_remove ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void raze::rpc_handler::account_representative ()
{
	std::string account_text (request.get<std::string> ("account"));
	raze::account account;
	auto error (account.decode_account (account_text));
...
void raze::rpc_handler::account_representative_set ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
			...
			if (existing != node.wallets.items.end ())
			{
				...
				if (!error)
				{
					...
					if (!error)
					{
						...
						{
							auto work_error (raze::from_string_hex (work_text.get (), work));
							if (work_error)
							{
						...
						if (work)
						{
							...
							if (!node.store.account_get (transaction, account, info))
							{
								if (!raze::work_validate (info.head, work))
								{
						...
						wallet->change_async (account, representative, [response_a](std::shared_ptr<raze::block> block) {
							raze::block_hash hash (0);
							if (block != nullptr)
							{
								hash = block->hash ();
							}
...
void raze::rpc_handler::account_weight ()
{
	std::string account_text (request.get<std::string> ("account"));
	raze::uint256_union account;
	auto error (account.decode_account (account_text));
...
void raze::rpc_handler::accounts_balances ()
{
	boost::property_tree::ptree response_l;
...
void raze::rpc_handler::accounts_create ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void raze::rpc_handler::accounts_frontiers ()
{
	boost::property_tree::ptree response_l;
...
void raze::rpc_handler::accounts_pending ()
{
	uint64_t count (std::numeric_limits<uint64_t>::max ());
	raze::uint128_union threshold (0);
	bool source (false);
	...
	for (auto & accounts : request.get_child ("accounts"))
	{
		...
		if (!account.decode_account (account_text))
		{
			...
			raze::account end (account.number () + 1);
			for (auto i (node.store.pending_begin (transaction, raze::pending_key (account, 0))), n (node.store.pending_begin (transaction, raze::pending_key (end, 0))); i != n && peers_l.size () < count; ++i)
			{
...
void raze::rpc_handler::available_supply ()
{
	auto genesis_balance (node.balance (raze::genesis_account)); // Cold storage genesis
	auto landing_balance (node.balance (raze::account ("059F68AAB29DE0D3A27443625C7EA9CDDB6517A8B76FE37727EF6A4D76832AD5"))); // Active unavailable account
	auto faucet_balance (node.balance (raze::account ("8E319CE6F3025E5B2DF66DA7AB1467FE48F1679C13DD43BFDB29FA2E9FC40D3B"))); // Faucet account
	auto burned_balance ((node.balance_pending (raze::account (0))).second); // Burning 0 account
	auto available (raze::genesis_amount - genesis_balance - landing_balance - faucet_balance - burned_balance);
	boost::property_tree::ptree response_l;
	response_l.put ("available", available.convert_to<std::string> ());
	response (response_l);
}

void raze::rpc_handler::block ()
{
	std::string hash_text (request.get<std::string> ("hash"));
	raze::uint256_union hash;
...
void raze::rpc_handler::blocks_info ()
{
	bool pending (false);
	boost::optional<bool> pending_optional (request.get_optional<bool> ("pending"));
	if (pending_optional.is_initialized ())
	...
	for (boost::property_tree::ptree::value_type & hashes : request.get_child ("hashes"))
	{
		...
		if (!error)
		{
			...
			if (block != nullptr)
			{
				...
				if (pending)
				{
					auto block_l (dynamic_cast<raze::send_block *> (block.get ()));
					bool exists (false);
					if (block_l != nullptr)
					{
						auto destination (block_l->hashables.destination);
						exists = node.store.pending_exists (transaction, raze::pending_key (destination, hash));
					}
					entry.put ("pending", exists ? "1" : "0");
				}
				if (source)
				{
					raze::block_hash source_hash (block->source ());
					std::unique_ptr<raze::block> block_a (node.store.block_get (transaction, source_hash));
					if (block_a != nullptr)
					{
...
void raze::rpc_handler::block_account ()
{
	std::string hash_text (request.get<std::string> ("hash"));
...
void raze::rpc_handler::block_count ()
{
	raze::transaction transaction (node.store.environment, nullptr, false);
	boost::property_tree::ptree response_l;
...
void raze::rpc_handler::block_count_type ()
{
	raze::transaction transaction (node.store.environment, nullptr, false);
	raze::block_counts count (node.store.block_count (transaction));
	boost::property_tree::ptree response_l;
...
void raze::rpc_handler::block_create ()
{
	if (rpc.config.enable_control)
	{
		std::string type (request.get<std::string> ("type"));
		raze::uint256_union wallet (0);
		...
		raze::uint256_union representative (0);
		...
		raze::uint256_union destination (0);
		...
		raze::raw_key prv;
		prv.data.clear ();
		raze::uint256_union previous (0);
		raze::uint128_union balance (0);
		...
		if (prv.data != 0)
		{
			...
			if (type == "open")
			{
				if (representative != 0 && source != 0)
				{
					...
					raze::open_block open (source, representative, pub, prv, pub, work);
					boost::property_tree::ptree response_l;
			...
			else if (type == "receive")
			{
				if (source != 0 && previous != 0)
				{
					...
					raze::receive_block receive (previous, source, prv, pub, work);
					boost::property_tree::ptree response_l;
			...
			else if (type == "change")
			{
				if (representative != 0 && previous != 0)
				{
					...
					raze::change_block change (previous, representative, prv, pub, work);
					boost::property_tree::ptree response_l;
			...
			else if (type == "send")
			{
				if (destination != 0 && previous != 0 && balance != 0 && amount != 0)
				{
					if (balance.number () >= amount.number ())
					{
						...
						raze::send_block send (previous, destination, balance.number () - amount.number (), prv, pub, work);
						boost::property_tree::ptree response_l;
...
void raze::rpc_handler::successors ()
{
	std::string block_text (request.get<std::string> ("block"));
	std::string count_text (request.get<std::string> ("count"));
	raze::block_hash block;
...
void raze::rpc_handler::bootstrap ()
{
	...
	{
		uint16_t port;
		if (!raze::parse_port (port_text, port))
		{
			node.bootstrap_initiator.bootstrap (raze::endpoint (address, port));
			boost::property_tree::ptree response_l;
...
void raze::rpc_handler::bootstrap_any ()
{
...
void raze::rpc_handler::delegators ()
{
	std::string account_text (request.get<std::string> ("account"));
	raze::account account;
	auto error (account.decode_account (account_text));
	if (!error)
	{
		...
		for (auto i (node.store.latest_begin (transaction)), n (node.store.latest_end ()); i != n; ++i)
		{
			...
			if (block->representative () == account)
			{
				std::string balance;
				raze::uint128_union (info.balance).encode_dec (balance);
				delegators.put (raze::account (i->first.uint256 ()).to_account (), balance);
			}
		}
...
void raze::rpc_handler::delegators_count ()
{
	std::string account_text (request.get<std::string> ("account"));
	raze::account account;
	auto error (account.decode_account (account_text));
...
void raze::rpc_handler::deterministic_key ()
{
	...
	if (!error)
	{
		...
		{
			raze::uint256_union index (index_a);
...
void raze::rpc_handler::frontiers ()
{
	std::string account_text (request.get<std::string> ("account"));
	std::string count_text (request.get<std::string> ("count"));
	raze::account start;
	if (!start.decode_account (account_text))
	{
		...
		if (!decode_unsigned (count_text, count))
		{
			...
			{
				frontiers.put (raze::account (i->first.uint256 ()).to_account (), raze::account_info (i->second).head.to_string ());
			}
			response_l.add_child ("frontiers", frontiers);
			response (response_l);
		}
...
void raze::rpc_handler::frontier_count ()
{
	raze::transaction transaction (node.store.environment, nullptr, false);
...
namespace
{
class history_visitor : public raze::block_visitor
{
public:
	history_visitor (raze::rpc_handler & handler_a, raze::transaction & transaction_a, boost::property_tree::ptree & tree_a, raze::block_hash const & hash_a) :
	handler (handler_a),
	...
	void open_block (raze::open_block const & block_a)
	{
		...
		{
			tree.put ("account", raze::genesis_account.to_account ());
			tree.put ("amount", raze::genesis_amount.convert_to<std::string> ());
		}
	}
	void change_block (raze::change_block const &)
	{
		// Don't report change blocks
	}
	raze::rpc_handler & handler;
	raze::transaction & transaction;
	boost::property_tree::ptree & tree;
	raze::block_hash const & hash;
};
}

void raze::rpc_handler::history ()
{
	std::string hash_text (request.get<std::string> ("hash"));
	std::string count_text (request.get<std::string> ("count"));
	raze::block_hash hash;
...
void raze::rpc_handler::account_history ()
{
	std::string account_text (request.get<std::string> ("account"));
	std::string count_text (request.get<std::string> ("count"));
	raze::uint256_union account;
...
void raze::rpc_handler::keepalive ()
{
	if (rpc.config.enable_control)
	{
		std::string address_text (request.get<std::string> ("address"));
...
void raze::rpc_handler::key_create ()
{
	boost::property_tree::ptree response_l;
	raze::keypair pair;
...
void raze::rpc_handler::key_expand ()
{
	std::string key_text (request.get<std::string> ("key"));
	raze::uint256_union prv;
...
void raze::rpc_handler::ledger ()
{
	if (rpc.config.enable_control)
	{
		raze::account start (0);
		uint64_t count (std::numeric_limits<uint64_t>::max ());
		bool sorting (false);
		boost::optional<std::string> account_text (request.get_optional<std::string> ("account"));
		if (account_text.is_initialized ())
		...
		{
			std::vector<std::pair<raze::uint128_union, raze::account>> ledger_l;
			for (auto i (node.store.latest_begin (transaction, start)), n (node.store.latest_end ()); i != n; ++i)
			{
				raze::uint128_union balance (raze::account_info (i->second).balance);
				ledger_l.push_back (std::make_pair (balance, raze::account (i->first.uint256 ())));
			}
...
void raze::rpc_handler::mraze_from_raw ()
{
...
void raze::rpc_handler::mraze_to_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	raze::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void raze::rpc_handler::kraze_from_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	raze::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void raze::rpc_handler::kraze_to_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	raze::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void raze::rpc_handler::password_change ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void raze::rpc_handler::password_enter ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	raze::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void raze::rpc_handler::password_valid (bool wallet_locked = false)
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	raze::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void raze::rpc_handler::pending_exists ()
{
	std::string hash_text (request.get<std::string> ("hash"));
	raze::uint256_union hash;
	auto error (hash.decode_hex (hash_text));
	if (!error)
	{
		...
		if (block != nullptr)
		{
			...
			if (block_l != nullptr)
			{
				...
				exists = node.store.pending_exists (transaction, raze::pending_key (account, hash));
			}
			boost::property_tree::ptree response_l;
...
void raze::rpc_handler::payment_begin ()
{
	...
	if (!id.decode_hex (id_text))
	{
		...
		{
			raze::transaction transaction (node.store.environment, nullptr, true);
			std::shared_ptr<raze::wallet> wallet (existing->second);
			if (wallet->store.valid_password (transaction))
			{
				raze::account account (0);
				do
				{
...
void raze::rpc_handler::payment_init ()
{
	std::string id_text (request.get<std::string> ("wallet"));
	raze::uint256_union id;
	if (!id.decode_hex (id_text))
...
void raze::rpc_handler::payment_end ()
{
	std::string id_text (request.get<std::string> ("wallet"));
	std::string account_text (request.get<std::string> ("account"));
	raze::uint256_union id;
...
void raze::rpc_handler::payment_wait ()
{
	std::string account_text (request.get<std::string> ("account"));
	std::string amount_text (request.get<std::string> ("amount"));
	...
	if (!account.decode_account (account_text))
	{
		...
		if (!amount.decode_dec (amount_text))
		{
			...
			{
				{
					auto observer (std::make_shared<raze::payment_observer> (response, rpc, account, amount));
...
void raze::rpc_handler::process ()
{
	...
	if (block != nullptr)
	{
		if (!raze::work_validate (*block))
		{
			auto hash (block->hash ());
			node.block_arrival.add (hash);
			raze::process_return result;
			std::shared_ptr<raze::block> block_a (std::move (block));
			{
				raze::transaction transaction (node.store.environment, nullptr, true);
...
void raze::rpc_handler::raze_from_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	raze::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void raze::rpc_handler::raze_to_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	raze::uint128_union amount;
	if (!amount.decode_dec (amount_text))
				...
				if (!error)
				{
					...
					if (account_check != existing->second->store.end ())
					{
						...
						if (!error)
						{
							...
							if (block != nullptr)
							{
								if (node.store.pending_exists (transaction, raze::pending_key (account, hash)))
								{
									uint64_t work (0);
									boost::optional<std::string> work_text (request.get_optional<std::string> ("work"));
									if (work_text.is_initialized ())
									...
									if (work)
									{
										...
										if (!raze::work_validate (head, work))
										{
											raze::transaction transaction_a (node.store.environment, nullptr, true);
									...
									auto response_a (response);
									existing->second->receive_async (std::move (block), account, raze::genesis_amount, [response_a](std::shared_ptr<raze::block> block_a) {
										raze::uint256_union hash_a (0);
										if (block_a != nullptr)
										{
											hash_a = block_a->hash ();
										}
...
void raze::rpc_handler::receive_minimum ()
{
	if (rpc.config.enable_control)
	{
		boost::property_tree::ptree response_l;
...
void raze::rpc_handler::receive_minimum_set ()
{
	if (rpc.config.enable_control)
	{
		std::string amount_text (request.get<std::string> ("amount"));
...
void raze::rpc_handler::representatives ()
{
	uint64_t count (std::numeric_limits<uint64_t>::max ());
	bool sorting (false);
	boost::optional<std::string> count_text (request.get_optional<std::string> ("count"));
	...
	{
		std::vector<std::pair<raze::uint128_union, std::string>> representation;
...
void raze::rpc_handler::republish ()
{
	...
	if (!error)
	{
		...
		if (block != nullptr)
		{
			for (auto i (0); !hash.is_zero () && i < count; ++i)
			{
				...
				{
					raze::block_hash source (block->source ());
					std::unique_ptr<raze::block> block_a (node.store.block_get (transaction, source));
					std::vector<raze::block_hash> hashes;
				...
				if (destinations != 0) // Republish destination chain
				{
					...
					auto block_s (dynamic_cast<raze::send_block *> (block_b.get ()));
					if (block_s != nullptr)
					{
						auto destination (block_s->hashables.destination);
						auto exists (node.store.pending_exists (transaction, raze::pending_key (destination, hash)));
						if (!exists)
						{
							raze::block_hash previous (node.ledger.latest (transaction, destination));
							std::unique_ptr<raze::block> block_d (node.store.block_get (transaction, previous));
...
void raze::rpc_handler::search_pending ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void raze::rpc_handler::search_pending_all ()
{
	if (rpc.config.enable_control)
	{
				...
				if (!error)
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							...
							raze::uint128_t balance (0);
							{
								raze::transaction transaction (node.store.environment, nullptr, work != 0); // false if no "work" in request, true if work > 0
								raze::account_info info;
							...
							if (balance >= amount.number ())
							{
								...
								auto response_a (response);
								existing->second->send_async (source, destination, amount.number (), [response_a](std::shared_ptr<raze::block> block_a) {
									if (block_a != nullptr)
									{
										raze::uint256_union hash (block_a->hash ());
										boost::property_tree::ptree response_l;
										response_l.put ("block", hash.to_string ());
										response_a (response_l);
									}
...
void raze::rpc_handler::unchecked ()
{
	uint64_t count (std::numeric_limits<uint64_t>::max ());
	boost::optional<std::string> count_text (request.get_optional<std::string> ("count"));
	if (count_text.is_initialized ())
...
void raze::rpc_handler::unchecked_clear ()
{
	if (rpc.config.enable_control)
	{
		raze::transaction transaction (node.store.environment, nullptr, true);
...
void raze::rpc_handler::unchecked_get ()
{
	std::string hash_text (request.get<std::string> ("hash"));
	raze::uint256_union hash;
	auto error (hash.decode_hex (hash_text));
...
void raze::rpc_handler::unchecked_keys ()
{
	uint64_t count (std::numeric_limits<uint64_t>::max ());
	...
	for (auto i (node.store.unchecked_begin (transaction, key)), n (node.store.unchecked_end ()); i != n && unchecked.size () < count; ++i)
	{
		...
		std::string contents;
		block->serialize_json (contents);
		entry.put ("key", raze::block_hash (i->first.uint256 ()).to_string ());
...
void raze::rpc_handler::version ()
{
	boost::property_tree::ptree response_l;
	response_l.put ("rpc_version", "1");
	response_l.put ("store_version", std::to_string (node.store_version ()));
	response_l.put ("node_vendor", boost::str (boost::format ("Raze %1%.%2%") % RAZE_VERSION_MAJOR % RAZE_VERSION_MINOR));
	response (response_l);
}

void raze::rpc_handler::validate_account_number ()
{
	std::string account_text (request.get<std::string> ("account"));
	raze::uint256_union account;
	auto error (account.decode_account (account_text));
...
void raze::rpc_handler::wallet_add ()
{
	if (rpc.config.enable_control)
	{
		std::string key_text (request.get<std::string> ("key"));
...
void raze::rpc_handler::wallet_balance_total ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	raze::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void raze::rpc_handler::wallet_balances ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	raze::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
	if (!error)
	{
		...
		if (existing != node.wallets.items.end ())
		{
			...
			{
				raze::account account (i->first.uint256 ());
				raze::uint128_t balance = node.ledger.account_balance (transaction, account);
				if (threshold.is_zero ())
				{
					boost::property_tree::ptree entry;
					raze::uint128_t pending = node.ledger.account_pending (transaction, account);
...
void raze::rpc_handler::wallet_change_seed ()
{
	if (rpc.config.enable_control)
	{
		std::string seed_text (request.get<std::string> ("seed"));
...
void raze::rpc_handler::wallet_contains ()
{
	std::string account_text (request.get<std::string> ("account"));
	std::string wallet_text (request.get<std::string> ("wallet"));
	raze::uint256_union account;
...
void raze::rpc_handler::wallet_create ()
{
	if (rpc.config.enable_control)
	{
		raze::keypair wallet_id;
...
void raze::rpc_handler::wallet_destroy ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void raze::rpc_handler::wallet_export ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	raze::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void raze::rpc_handler::wallet_frontiers ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	raze::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void raze::rpc_handler::wallet_key_valid ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	raze::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void raze::rpc_handler::wallet_lock ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void raze::rpc_handler::wallet_pending ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	raze::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
	if (!error)
	{
		...
		if (existing != node.wallets.items.end ())
		{
			...
			for (auto i (existing->second->store.begin (transaction)), n (existing->second->store.end ()); i != n; ++i)
			{
				...
				boost::property_tree::ptree peers_l;
				raze::account end (account.number () + 1);
				for (auto ii (node.store.pending_begin (transaction, raze::pending_key (account, 0))), nn (node.store.pending_begin (transaction, raze::pending_key (end, 0))); ii != nn && peers_l.size () < count; ++ii)
				{
					raze::pending_key key (ii->first);
					if (threshold.is_zero () && !source)
					{
						boost::property_tree::ptree entry;
						entry.put ("", key.hash.to_string ());
					...
					{
						raze::pending_info info (ii->second);
						if (info.amount.number () >= threshold.number ())
						{
							if (source)
							{
...
void raze::rpc_handler::wallet_representative ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	raze::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void raze::rpc_handler::wallet_representative_set ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void raze::rpc_handler::wallet_republish ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
			...
			if (existing != node.wallets.items.end ())
			{
				...
				if (!error)
				{
					...
					for (auto i (existing->second->store.begin (transaction)), n (existing->second->store.end ()); i != n; ++i)
					{
						raze::account account (i->first.uint256 ());
						auto latest (node.ledger.latest (transaction, account));
						std::unique_ptr<raze::block> block;
						std::vector<raze::block_hash> hashes;
...
void raze::rpc_handler::wallet_work_get ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
		...
		if (!error)
		{
			...
			if (existing != node.wallets.items.end ())
			{
				...
				for (auto i (existing->second->store.begin (transaction)), n (existing->second->store.end ()); i != n; ++i)
				{
					...
					works.put (account.to_account (), raze::to_string_hex (work));
				}
...
void raze::rpc_handler::work_generate ()
{
	if (rpc.config.enable_control)
	{
		std::string hash_text (request.get<std::string> ("hash"));
		...
		if (!error)
		{
			...
			node.work.generate (hash, [rpc_l](boost::optional<uint64_t> const & work_a) {
				...
				{
					boost::property_tree::ptree response_l;
					response_l.put ("work", raze::to_string_hex (work_a.value ()));
...
void raze::rpc_handler::work_cancel ()
{
	if (rpc.config.enable_control)
	{
		std::string hash_text (request.get<std::string> ("hash"));
...
void raze::rpc_handler::work_get ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
			...
			if (existing != node.wallets.items.end ())
			{
				...
				if (!error)
				{
					...
					if (account_check != existing->second->store.end ())
					{
						...
						auto error_work (existing->second->store.work_get (transaction, account, work));
						boost::property_tree::ptree response_l;
						response_l.put ("work", raze::to_string_hex (work));
						response (response_l);
					}
					else
					{
...
void raze::rpc_handler::work_set ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
			...
			if (existing != node.wallets.items.end ())
			{
				...
				if (!error)
				{
					...
					if (account_check != existing->second->store.end ())
					{
						...
						uint64_t work;
						auto work_error (raze::from_string_hex (work_text, work));
						if (!work_error)
						{
							existing->second->store.work_put (transaction, account, work);
							boost::property_tree::ptree response_l;
...
void raze::rpc_handler::work_validate ()
{
	std::string hash_text (request.get<std::string> ("hash"));
	raze::block_hash hash;
	auto error (hash.decode_hex (hash_text));
	if (!error)
	{
		...
		if (!work_error)
		{
			auto validate (raze::work_validate (hash, work));
			boost::property_tree::ptree response_l;
...
void raze::rpc_handler::work_peer_add ()
{
	if (rpc.config.enable_control)
	{
		std::string address_text = request.get<std::string> ("address");
...
void raze::rpc_handler::work_peers ()
{
	if (rpc.config.enable_control)
	{
...
void raze::rpc_handler::work_peers_clear ()
{
	if (rpc.config.enable_control)
	{
...
raze::rpc_connection::rpc_connection (raze::node & node_a, raze::rpc & rpc_a) :
node (node_a.shared ()),
rpc (rpc_a),
...
void raze::rpc_connection::parse_connection ()
{
	...
	boost::beast::http::async_read (socket, buffer, request, [this_l](boost::system::error_code const & ec, size_t bytes_transferred) {
		if (!ec)
		{
			this_l->node->background ([this_l]() {
				...
				{
					auto handler (std::make_shared<raze::rpc_handler> (*this_l->node, this_l->rpc, this_l->request.body (), response_handler));
...
void raze::rpc_handler::process_request ()
{
	try
	{
		...
		else if (action == "kraze_from_raw")
		{
			kraze_from_raw ();
		}
		else if (action == "kraze_to_raw")
		{
			kraze_to_raw ();
		}
		...
		else if (action == "mraze_from_raw")
		{
			mraze_from_raw ();
		}
		else if (action == "mraze_to_raw")
		{
			mraze_to_raw ();
		}
		else if (action == "password_change")
		...
		else if (action == "raze_from_raw")
		{
			raze_from_raw ();
		}
		else if (action == "raze_to_raw")
		{
			raze_to_raw ();
		}
...
raze::payment_observer::payment_observer (std::function<void(boost::property_tree::ptree const &)> const & response_a, raze::rpc & rpc_a, raze::account const & account_a, raze::amount const & amount_a) :
rpc (rpc_a),
account (account_a),
amount (amount_a),
response (response_a)
...
void raze::payment_observer::start (uint64_t timeout)
{
	auto this_l (shared_from_this ());
	rpc.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::milliseconds (timeout), [this_l]() {
		this_l->complete (raze::payment_status::nothing);
	});
}

raze::payment_observer::~payment_observer ()
{
}

void raze::payment_observer::observe ()
{
	if (rpc.node.balance (account) >= amount.number ())
	{
		complete (raze::payment_status::success);
	}
}

void raze::payment_observer::complete (raze::payment_status status)
{
	...
	if (!already)
	{
		...
		switch (status)
		{
			case raze::payment_status::nothing:
			{
				boost::property_tree::ptree response_l;
			...
			case raze::payment_status::success:
			{
				boost::property_tree::ptree response_l;
