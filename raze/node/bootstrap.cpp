...
namespace
{
class add_dependency_visitor : public raze::block_visitor
{
public:
	add_dependency_visitor (MDB_txn * transaction_a, raze::block_synchronization & sync_a) :
	...
	void add_dependency (raze::block_hash const & hash_a)
	{
	...
	MDB_txn * transaction;
	raze::block_synchronization & sync;
	bool complete;
};
}

bool raze::block_synchronization::add_dependency (MDB_txn * transaction_a, raze::block const & block_a)
{
...
void raze::block_synchronization::fill_dependencies (MDB_txn * transaction_a)
{
	auto done (false);
	while (!done)
	{
...
raze::sync_result raze::block_synchronization::synchronize_one (MDB_txn * transaction_a)
{
	...
	raze::sync_result result (raze::sync_result::success);
	auto hash (blocks.back ());
	blocks.pop_back ();
	auto block (retrieve (transaction_a, hash));
	if (block != nullptr)
...
raze::sync_result raze::block_synchronization::synchronize (MDB_txn * transaction_a, raze::block_hash const & hash_a)
{
	auto result (raze::sync_result::success);
	blocks.clear ();
	blocks.push_back (hash_a);
	auto cutoff (std::chrono::steady_clock::now () + raze::transaction_timeout);
	while (std::chrono::steady_clock::now () < cutoff && result != raze::sync_result::fork && !blocks.empty ())
	{
...
raze::push_synchronization::push_synchronization (raze::node & node_a, std::function<raze::sync_result (MDB_txn *, raze::block const &)> const & target_a) :
...
bool raze::push_synchronization::synchronized (MDB_txn * transaction_a, raze::block_hash const & hash_a)
{
...
std::unique_ptr<raze::block> raze::push_synchronization::retrieve (MDB_txn * transaction_a, raze::block_hash const & hash_a)
{
	return node.store.block_get (transaction_a, hash_a);
}

raze::sync_result raze::push_synchronization::target (MDB_txn * transaction_a, raze::block const & block_a)
{
	return target_m (transaction_a, block_a);
}

raze::bootstrap_client::bootstrap_client (std::shared_ptr<raze::node> node_a, std::shared_ptr<raze::bootstrap_attempt> attempt_a, raze::tcp_endpoint const & endpoint_a) :
node (node_a),
attempt (attempt_a),
...
void raze::bootstrap_client::start_timeout ()
{
	timeout.expires_from_now (boost::posix_time::seconds (5));
	std::weak_ptr<raze::bootstrap_client> this_w (shared ());
...
void raze::bootstrap_client::stop_timeout ()
{
	size_t killed (timeout.cancel ());
	(void)killed;
}

void raze::bootstrap_client::run ()
{
...
void raze::frontier_req_client::run ()
{
	std::unique_ptr<raze::frontier_req> request (new raze::frontier_req);
	request->start.clear ();
	...
	{
		raze::vectorstream stream (*send_buffer);
...
std::shared_ptr<raze::bootstrap_client> raze::bootstrap_client::shared ()
{
	return shared_from_this ();
}

raze::frontier_req_client::frontier_req_client (std::shared_ptr<raze::bootstrap_client> connection_a) :
connection (connection_a),
current (0),
count (0),
next_report (std::chrono::steady_clock::now () + std::chrono::seconds (15))
{
	raze::transaction transaction (connection->node->store.environment, nullptr, false);
	next (transaction);
}

raze::frontier_req_client::~frontier_req_client ()
{
}

void raze::frontier_req_client::receive_frontier ()
{
	auto this_l (shared_from_this ());
	connection->start_timeout ();
	size_t size_l (sizeof (raze::uint256_union) + sizeof (raze::uint256_union));
...
void raze::frontier_req_client::unsynced (MDB_txn * transaction_a, raze::block_hash const & ours_a, raze::block_hash const & theirs_a)
{
	auto current (ours_a);
...
void raze::frontier_req_client::received_frontier (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == sizeof (raze::uint256_union) + sizeof (raze::uint256_union));
		raze::account account;
		raze::bufferstream account_stream (connection->receive_buffer.data (), sizeof (raze::uint256_union));
		auto error1 (raze::read (account_stream, account));
		assert (!error1);
		raze::block_hash latest;
		raze::bufferstream latest_stream (connection->receive_buffer.data () + sizeof (raze::uint256_union), sizeof (raze::uint256_union));
		auto error2 (raze::read (latest_stream, latest));
		assert (!error2);
		if (count == 0)
		{
			start_time = std::chrono::steady_clock::now ();
		...
		if (!account.is_zero ())
		{
			while (!current.is_zero () && current < account)
			{
				...
				raze::transaction transaction (connection->node->store.environment, nullptr, true);
			...
			if (!current.is_zero ())
			{
				if (account == current)
				{
					...
					else
					{
						...
						{
							connection->attempt->add_pull (raze::pull_info (account, latest, info.head));
						}
					}
					next (transaction);
				}
				else
				{
					...
					connection->attempt->add_pull (raze::pull_info (account, latest, raze::block_hash (0)));
				}
			}
			else
			{
...
void raze::frontier_req_client::next (MDB_txn * transaction_a)
{
	auto iterator (connection->node->store.latest_begin (transaction_a, raze::uint256_union (current.number () + 1)));
	if (iterator != connection->node->store.latest_end ())
	{
		current = raze::account (iterator->first.uint256 ());
		info = raze::account_info (iterator->second);
	}
	else
	{
		current.clear ();
...
raze::bulk_pull_client::bulk_pull_client (std::shared_ptr<raze::bootstrap_client> connection_a) :
connection (connection_a)
{
...
raze::bulk_pull_client::~bulk_pull_client ()
{
	{
...
void raze::bulk_pull_client::request (raze::pull_info const & pull_a)
{
	pull = pull_a;
	...
	{
		raze::vectorstream stream (*buffer);
...
void raze::bulk_pull_client::receive_block ()
{
	auto this_l (shared_from_this ());
	connection->start_timeout ();
...
void raze::bulk_pull_client::received_type ()
{
	auto this_l (shared_from_this ());
	raze::block_type type (static_cast<raze::block_type> (connection->receive_buffer[0]));
	switch (type)
	{
		case raze::block_type::send:
		{
			connection->start_timeout ();
			boost::asio::async_read (connection->socket, boost::asio::buffer (connection->receive_buffer.data () + 1, raze::send_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->connection->stop_timeout ();
		...
		{
			connection->start_timeout ();
			boost::asio::async_read (connection->socket, boost::asio::buffer (connection->receive_buffer.data () + 1, raze::receive_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->connection->stop_timeout ();
				this_l->received_block (ec, size_a);
			});
			break;
		...
		{
			connection->start_timeout ();
			boost::asio::async_read (connection->socket, boost::asio::buffer (connection->receive_buffer.data () + 1, raze::open_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->connection->stop_timeout ();
				this_l->received_block (ec, size_a);
			});
			break;
		...
		{
			connection->start_timeout ();
			boost::asio::async_read (connection->socket, boost::asio::buffer (connection->receive_buffer.data () + 1, raze::change_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->connection->stop_timeout ();
				this_l->received_block (ec, size_a);
			});
			break;
		}
		case raze::block_type::not_a_block:
		{
...
void raze::bulk_pull_client::received_block (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		...
		if (block != nullptr && !raze::work_validate (*block))
		{
			...
			connection->attempt->node->block_processor.add (raze::block_processor_item (block));
...
raze::bulk_push_client::bulk_push_client (std::shared_ptr<raze::bootstrap_client> const & connection_a) :
connection (connection_a),
synchronization (*connection->node, [this](MDB_txn * transaction_a, raze::block const & block_a) {
	push_block (block_a);
	return raze::sync_result::success;
})
{
}

raze::bulk_push_client::~bulk_push_client ()
{
}

void raze::bulk_push_client::start ()
{
	raze::bulk_push message;
	auto buffer (std::make_shared<std::vector<uint8_t>> ());
	{
		raze::vectorstream stream (*buffer);
	...
	boost::asio::async_write (connection->socket, boost::asio::buffer (buffer->data (), buffer->size ()), [this_l, buffer](boost::system::error_code const & ec, size_t size_a) {
		this_l->connection->stop_timeout ();
		raze::transaction transaction (this_l->connection->node->store.environment, nullptr, true);
		if (!ec)
		{
...
void raze::bulk_push_client::push (MDB_txn * transaction_a)
{
	auto finished (false);
	{
		auto first (connection->node->store.unsynced_begin (transaction_a));
		if (first != raze::store_iterator (nullptr))
		{
			raze::block_hash hash (first->first.uint256 ());
			if (!hash.is_zero ())
			{
...
void raze::bulk_push_client::send_finished ()
{
	auto buffer (std::make_shared<std::vector<uint8_t>> ());
	buffer->push_back (static_cast<uint8_t> (raze::block_type::not_a_block));
	if (connection->node->config.logging.network_logging ())
	{
...
void raze::bulk_push_client::push_block (raze::block const & block_a)
{
	auto buffer (std::make_shared<std::vector<uint8_t>> ());
	{
		raze::vectorstream stream (*buffer);
...
raze::pull_info::pull_info (raze::account const & account_a, raze::block_hash const & head_a, raze::block_hash const & end_a) :
account (account_a),
head (head_a),
end (end_a),
attempts (0)
...
raze::bootstrap_attempt::bootstrap_attempt (std::shared_ptr<raze::node> node_a) :
connections (0),
pulling (0),
node (node_a),
account_count (0),
...
raze::bootstrap_attempt::~bootstrap_attempt ()
{
	BOOST_LOG (node->log) << "Exiting bootstrap attempt";
	node->bootstrap_initiator.notify_listeners (false);
}

bool raze::bootstrap_attempt::request_frontier (std::unique_lock<std::mutex> & lock_a)
{
	auto result (true);
	...
	if (connection_l)
	{
		...
		{
			auto client (std::make_shared<raze::frontier_req_client> (connection_l));
			client->run ();
			frontiers = client;
...
void raze::bootstrap_attempt::request_pull (std::unique_lock<std::mutex> & lock_a)
{
	auto connection_l (connection (lock_a));
	if (connection_l)
	{
		...
		pulls.pop_front ();
		auto client (std::make_shared<raze::bulk_pull_client> (connection_l));
...
bool raze::bootstrap_attempt::request_push (std::unique_lock<std::mutex> & lock_a)
{
	auto result (true);
	auto connection_l (connection (lock_a));
	if (connection_l)
	{
		std::future<bool> future;
		{
			auto client (std::make_shared<raze::bulk_push_client> (connection_l));
			client->start ();
			push = client;
			future = client->promise.get_future ();
		}
...
bool raze::bootstrap_attempt::still_pulling ()
{
...
void raze::bootstrap_attempt::run ()
{
	...
	{
		auto k = raze::random_pool.GenerateWord32 (0, i);
...
std::shared_ptr<raze::bootstrap_client> raze::bootstrap_attempt::connection (std::unique_lock<std::mutex> & lock_a)
{
	while (!stopped && idle.empty ())
	{
		condition.wait (lock_a);
	}
	std::shared_ptr<raze::bootstrap_client> result;
	if (!idle.empty ())
	{
		result = idle.back ();
		idle.pop_back ();
...
bool raze::bootstrap_attempt::consume_future (std::future<bool> & future_a)
{
	bool result;
	try
	{
...
void raze::bootstrap_attempt::process_fork (MDB_txn * transaction_a, std::shared_ptr<raze::block> block_a)
{
	try_resolve_fork (transaction_a, block_a, true);
}

void raze::bootstrap_attempt::try_resolve_fork (MDB_txn * transaction_a, std::shared_ptr<raze::block> block_a, bool from_processor)
{
	std::weak_ptr<raze::bootstrap_attempt> this_w (shared_from_this ());
	std::shared_ptr<raze::block> ledger_block (node->ledger.forked_block (transaction_a, *block_a));
	if (ledger_block)
	{
		node->active.start (transaction_a, ledger_block, [this_w, block_a](std::shared_ptr<raze::block>, bool resolved) {
			if (auto this_l = this_w.lock ())
			{
				if (resolved)
				{
					...
					raze::transaction transaction (this_l->node->store.environment, nullptr, false);
					auto account (this_l->node->ledger.store.frontier_get (transaction, block_a->root ()));
					if (!account.is_zero ())
					{
						this_l->requeue_pull (raze::pull_info (account, block_a->root (), block_a->root ()));
					}
					else if (this_l->node->ledger.store.account_exists (transaction, block_a->root ()))
					{
						this_l->requeue_pull (raze::pull_info (block_a->root (), raze::block_hash (0), raze::block_hash (0)));
					}
				}
			}
		});
...
void raze::bootstrap_attempt::resolve_forks ()
{
	std::unordered_map<raze::block_hash, std::shared_ptr<raze::block>> forks_to_resolve;
	{
		std::unique_lock<std::mutex> lock (mutex);
...
{
	bool operator() (const std::shared_ptr<raze::bootstrap_client> & lhs, const std::shared_ptr<raze::bootstrap_client> & rhs) const
	{
		return lhs->block_rate () > rhs->block_rate ();
	}
};

unsigned raze::bootstrap_attempt::target_connections (size_t pulls_remaining)
{
...
void raze::bootstrap_attempt::populate_connections ()
{
	double rate_sum = 0.0;
	size_t num_pulls = 0;
	std::priority_queue<std::shared_ptr<raze::bootstrap_client>, std::vector<std::shared_ptr<raze::bootstrap_client>>, block_rate_cmp> sorted_connections;
	{
		std::unique_lock<std::mutex> lock (mutex);
	...
	if (connections < target)
	{
		...
		for (int i = 0; i < delta; i++)
		{
			...
			if (peer != raze::endpoint (boost::asio::ip::address_v6::any (), 0))
			{
				auto client (std::make_shared<raze::bootstrap_client> (node, shared_from_this (), raze::tcp_endpoint (peer.address (), peer.port ())));
				client->run ();
...
void raze::bootstrap_attempt::add_connection (raze::endpoint const & endpoint_a)
{
	auto client (std::make_shared<raze::bootstrap_client> (node, shared_from_this (), raze::tcp_endpoint (endpoint_a.address (), endpoint_a.port ())));
	client->run ();
}

void raze::bootstrap_attempt::pool_connection (std::shared_ptr<raze::bootstrap_client> client_a)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void raze::bootstrap_attempt::stop ()
{
	std::lock_guard<std::mutex> lock (mutex);
	stopped = true;
	condition.notify_all ();
...
void raze::bootstrap_attempt::add_pull (raze::pull_info const & pull)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void raze::bootstrap_attempt::requeue_pull (raze::pull_info const & pull_a)
{
	auto pull (pull_a);
	...
	else if (pull.attempts == bootstrap_frontier_retry_limit)
	{
		...
		{
			auto client (std::make_shared<raze::bulk_pull_client> (connection_shared));
			node->background ([client, pull]() {
				client->request (pull);
			});
			if (node->config.logging.bulk_pull_logging ())
...
raze::bootstrap_initiator::bootstrap_initiator (raze::node & node_a) :
node (node_a),
stopped (false),
...
void raze::bootstrap_initiator::bootstrap ()
{
	std::unique_lock<std::mutex> lock (mutex);
	if (!stopped && attempt == nullptr)
	{
		attempt = std::make_shared<raze::bootstrap_attempt> (node.shared ());
		condition.notify_all ();
	}
}

void raze::bootstrap_initiator::bootstrap (raze::endpoint const & endpoint_a)
{
...
void raze::bootstrap_initiator::run_bootstrap ()
{
	std::unique_lock<std::mutex> lock (mutex);
	while (!stopped)
	{
...
void raze::bootstrap_initiator::add_observer (std::function<void(bool)> const & observer_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	observers.push_back (observer_a);
}

bool raze::bootstrap_initiator::in_progress ()
{
	std::lock_guard<std::mutex> lock (mutex);
	return attempt != nullptr;
}

void raze::bootstrap_initiator::stop ()
{
	std::unique_lock<std::mutex> lock (mutex);
	stopped = true;
	if (attempt != nullptr)
...
void raze::bootstrap_initiator::notify_listeners (bool in_progress_a)
{
...
void raze::bootstrap_initiator::process_fork (MDB_txn * transaction, std::shared_ptr<raze::block> block_a)
{
	std::unique_lock<std::mutex> lock (mutex);
	if (attempt != nullptr)
	{
...
raze::bootstrap_listener::bootstrap_listener (boost::asio::io_service & service_a, uint16_t port_a, raze::node & node_a) :
acceptor (service_a),
...
void raze::bootstrap_listener::start ()
{
...
void raze::bootstrap_listener::stop ()
{
	on = false;
	std::lock_guard<std::mutex> lock (mutex);
	acceptor.close ();
...
void raze::bootstrap_listener::accept_connection ()
{
...
void raze::bootstrap_listener::accept_action (boost::system::error_code const & ec, std::shared_ptr<boost::asio::ip::tcp::socket> socket_a)
{
	if (!ec)
	{
		accept_connection ();
		auto connection (std::make_shared<raze::bootstrap_server> (socket_a, node.shared ()));
		{
			std::lock_guard<std::mutex> lock (mutex);
			if (acceptor.is_open ())
			{
...
boost::asio::ip::tcp::endpoint raze::bootstrap_listener::endpoint ()
{
	return boost::asio::ip::tcp::endpoint (boost::asio::ip::address_v6::loopback (), local.port ());
}

raze::bootstrap_server::~bootstrap_server ()
{
	if (node->config.logging.bulk_pull_logging ())
	{
...
raze::bootstrap_server::bootstrap_server (std::shared_ptr<boost::asio::ip::tcp::socket> socket_a, std::shared_ptr<raze::node> node_a) :
socket (socket_a),
node (node_a)
{
}

void raze::bootstrap_server::receive ()
{
	auto this_l (shared_from_this ());
...
void raze::bootstrap_server::receive_header_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == 8);
		raze::bufferstream type_stream (receive_buffer.data (), size_a);
		uint8_t version_max;
		uint8_t version_using;
		uint8_t version_min;
		raze::message_type type;
		std::bitset<16> extensions;
		if (!raze::message::read_header (type_stream, version_max, version_using, version_min, type, extensions))
		{
			switch (type)
			{
				case raze::message_type::bulk_pull:
				{
					auto this_l (shared_from_this ());
					boost::asio::async_read (*socket, boost::asio::buffer (receive_buffer.data () + 8, sizeof (raze::uint256_union) + sizeof (raze::uint256_union)), [this_l](boost::system::error_code const & ec, size_t size_a) {
						this_l->receive_bulk_pull_action (ec, size_a);
					});
					break;
				}
				case raze::message_type::bulk_pull_blocks:
				{
					auto this_l (shared_from_this ());
					boost::asio::async_read (*socket, boost::asio::buffer (receive_buffer.data () + raze::bootstrap_message_header_size, sizeof (raze::uint256_union) + sizeof (raze::uint256_union) + sizeof (bulk_pull_blocks_mode) + sizeof (uint32_t)), [this_l](boost::system::error_code const & ec, size_t size_a) {
						this_l->receive_bulk_pull_blocks_action (ec, size_a);
					});
					break;
				}
				case raze::message_type::frontier_req:
				{
					auto this_l (shared_from_this ());
					boost::asio::async_read (*socket, boost::asio::buffer (receive_buffer.data () + 8, sizeof (raze::uint256_union) + sizeof (uint32_t) + sizeof (uint32_t)), [this_l](boost::system::error_code const & ec, size_t size_a) {
						this_l->receive_frontier_req_action (ec, size_a);
					});
					break;
				}
				case raze::message_type::bulk_push:
				{
					add_request (std::unique_ptr<raze::message> (new raze::bulk_push));
					break;
				}
				default:
				{
...
void raze::bootstrap_server::receive_bulk_pull_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		std::unique_ptr<raze::bulk_pull> request (new raze::bulk_pull);
		raze::bufferstream stream (receive_buffer.data (), 8 + sizeof (raze::uint256_union) + sizeof (raze::uint256_union));
		...
		if (!error)
		{
			...
			add_request (std::unique_ptr<raze::message> (request.release ()));
			receive ();
		}
	}
}

void raze::bootstrap_server::receive_bulk_pull_blocks_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		std::unique_ptr<raze::bulk_pull_blocks> request (new raze::bulk_pull_blocks);
		raze::bufferstream stream (receive_buffer.data (), 8 + sizeof (raze::uint256_union) + sizeof (raze::uint256_union) + sizeof (bulk_pull_blocks_mode) + sizeof (uint32_t));
		auto error (request->deserialize (stream));
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
...
void raze::bootstrap_server::receive_frontier_req_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		std::unique_ptr<raze::frontier_req> request (new raze::frontier_req);
		raze::bufferstream stream (receive_buffer.data (), 8 + sizeof (raze::uint256_union) + sizeof (uint32_t) + sizeof (uint32_t));
		auto error (request->deserialize (stream));
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
...
void raze::bootstrap_server::add_request (std::unique_ptr<raze::message> message_a)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void raze::bootstrap_server::finish_request ()
{
	std::lock_guard<std::mutex> lock (mutex);
	requests.pop ();
	if (!requests.empty ())
...
namespace
{
class request_response_visitor : public raze::message_visitor
{
public:
	request_response_visitor (std::shared_ptr<raze::bootstrap_server> connection_a) :
	connection (connection_a)
	{
	}
	virtual ~request_response_visitor () = default;
	void keepalive (raze::keepalive const &) override
	{
		assert (false);
	}
	void publish (raze::publish const &) override
	{
		assert (false);
	}
	void confirm_req (raze::confirm_req const &) override
	{
		assert (false);
	}
	void confirm_ack (raze::confirm_ack const &) override
	{
		assert (false);
	}
	void bulk_pull (raze::bulk_pull const &) override
	{
		auto response (std::make_shared<raze::bulk_pull_server> (connection, std::unique_ptr<raze::bulk_pull> (static_cast<raze::bulk_pull *> (connection->requests.front ().release ()))));
		response->send_next ();
	}
	void bulk_pull_blocks (raze::bulk_pull_blocks const &) override
	{
		auto response (std::make_shared<raze::bulk_pull_blocks_server> (connection, std::unique_ptr<raze::bulk_pull_blocks> (static_cast<raze::bulk_pull_blocks *> (connection->requests.front ().release ()))));
		response->send_next ();
	}
	void bulk_push (raze::bulk_push const &) override
	{
		auto response (std::make_shared<raze::bulk_push_server> (connection));
		response->receive ();
	}
	void frontier_req (raze::frontier_req const &) override
	{
		auto response (std::make_shared<raze::frontier_req_server> (connection, std::unique_ptr<raze::frontier_req> (static_cast<raze::frontier_req *> (connection->requests.front ().release ()))));
		response->send_next ();
	}
...
void raze::bootstrap_server::run_next ()
{
 ...
 */
void raze::bulk_pull_server::set_current_end ()
{
...
void raze::bulk_pull_server::send_next ()
{
	std::unique_ptr<raze::block> block (get_next ());
	if (block != nullptr)
	{
		{
			send_buffer.clear ();
			raze::vectorstream stream (send_buffer);
			raze::serialize_block (stream, *block);
		}
		auto this_l (shared_from_this ());
		if (connection->node->config.logging.bulk_pull_logging ())
		{
...
std::unique_ptr<raze::block> raze::bulk_pull_server::get_next ()
{
...
void raze::bulk_pull_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next ();
...
void raze::bulk_pull_server::send_finished ()
{
	send_buffer.clear ();
	send_buffer.push_back (static_cast<uint8_t> (raze::block_type::not_a_block));
	auto this_l (shared_from_this ());
	if (connection->node->config.logging.bulk_pull_logging ())
	{
...
void raze::bulk_pull_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == 1);
...
raze::bulk_pull_server::bulk_pull_server (std::shared_ptr<raze::bootstrap_server> const & connection_a, std::unique_ptr<raze::bulk_pull> request_a) :
connection (connection_a),
 ...
 */
void raze::bulk_pull_blocks_server::set_params ()
{
	assert (request != nullptr);
	...
	if (connection->node->config.logging.bulk_pull_logging ())
	{
		...
		switch (request->mode)
		{
			case raze::bulk_pull_blocks_mode::list_blocks:
				modeName = "list";
				break;
			case raze::bulk_pull_blocks_mode::checksum_blocks:
				modeName = "checksum";
				break;
		}
...
void raze::bulk_pull_blocks_server::send_next ()
{
	std::unique_ptr<raze::block> block (get_next ());
	if (block != nullptr)
	{
		...
		if (request->mode == raze::bulk_pull_blocks_mode::list_blocks)
		{
			raze::vectorstream stream (send_buffer);
			raze::serialize_block (stream, *block);
		}
		else if (request->mode == raze::bulk_pull_blocks_mode::checksum_blocks)
		{
			checksum ^= block->hash ();
		}
	...
	else
	{
		...
		if (request->mode == raze::bulk_pull_blocks_mode::checksum_blocks)
		{
			{
				send_buffer.clear ();
				raze::vectorstream stream (send_buffer);
				write (stream, static_cast<uint8_t> (raze::block_type::not_a_block));
...
std::unique_ptr<raze::block> raze::bulk_pull_blocks_server::get_next ()
{
	std::unique_ptr<raze::block> result;
	bool out_of_bounds;
...
void raze::bulk_pull_blocks_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next ();
...
void raze::bulk_pull_blocks_server::send_finished ()
{
	send_buffer.clear ();
	send_buffer.push_back (static_cast<uint8_t> (raze::block_type::not_a_block));
	auto this_l (shared_from_this ());
...
void raze::bulk_pull_blocks_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == 1);
...
raze::bulk_pull_blocks_server::bulk_pull_blocks_server (std::shared_ptr<raze::bootstrap_server> const & connection_a, std::unique_ptr<raze::bulk_pull_blocks> request_a) :
connection (connection_a),
request (std::move (request_a)),
stream (nullptr),
...
raze::bulk_push_server::bulk_push_server (std::shared_ptr<raze::bootstrap_server> const & connection_a) :
connection (connection_a)
{
}

void raze::bulk_push_server::receive ()
{
	auto this_l (shared_from_this ());
...
void raze::bulk_push_server::received_type ()
{
	auto this_l (shared_from_this ());
	raze::block_type type (static_cast<raze::block_type> (receive_buffer[0]));
	switch (type)
	{
		case raze::block_type::send:
		{
			boost::asio::async_read (*connection->socket, boost::asio::buffer (receive_buffer.data () + 1, raze::send_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a);
			});
			break;
		}
		case raze::block_type::receive:
		{
			boost::asio::async_read (*connection->socket, boost::asio::buffer (receive_buffer.data () + 1, raze::receive_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a);
			});
			break;
		}
		case raze::block_type::open:
		{
			boost::asio::async_read (*connection->socket, boost::asio::buffer (receive_buffer.data () + 1, raze::open_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a);
			});
			break;
		}
		case raze::block_type::change:
		{
			boost::asio::async_read (*connection->socket, boost::asio::buffer (receive_buffer.data () + 1, raze::change_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a);
			});
			break;
		}
...
void raze::bulk_push_server::received_block (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
...
raze::frontier_req_server::frontier_req_server (std::shared_ptr<raze::bootstrap_server> const & connection_a, std::unique_ptr<raze::frontier_req> request_a) :
connection (connection_a),
...
void raze::frontier_req_server::skip_old ()
{
...
void raze::frontier_req_server::send_next ()
{
	if (!current.is_zero ())
	{
		{
...
void raze::frontier_req_server::send_finished ()
{
	{
		send_buffer.clear ();
		raze::vectorstream stream (send_buffer);
		raze::uint256_union zero (0);
...
void raze::frontier_req_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		connection->finish_request ();
...
void raze::frontier_req_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next ();
...
void raze::frontier_req_server::next ()
{
	raze::transaction transaction (connection->node->store.environment, nullptr, false);
	auto iterator (connection->node->store.latest_begin (transaction, current.number () + 1));
	if (iterator != connection->node->store.latest_end ())
	{
		current = raze::uint256_union (iterator->first.uint256 ());
		info = raze::account_info (iterator->second);
	}
	else
	{
