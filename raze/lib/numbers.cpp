...
thread_local CryptoPP::AutoSeededRandomPool raze::random_pool;
...
void raze::uint256_union::encode_account (std::string & destination_a) const
{
	...
	raze::uint512_t number_l (number ());
	number_l <<= 40;
	number_l |= raze::uint512_t (check);
	for (auto i (0); i < 60; ++i)
	{
	...
	destination_a.append ("_brx"); // raze_
	std::reverse (destination_a.begin (), destination_a.end ());
}

std::string raze::uint256_union::to_account_split () const
{
...
std::string raze::uint256_union::to_account () const
{
	std::string result;
...
bool raze::uint256_union::decode_account_v1 (std::string const & source_a)
{
	auto error (source_a.size () != 50);
	if (!error)
	{
		raze::uint512_t number_l;
		...
		if (!error)
		{
			*this = number_l.convert_to<raze::uint256_t> ();
...
bool raze::uint256_union::decode_account (std::string const & source_a)
{
	...
	if (!error)
	{
		if (source_a[0] == 'x' && source_a[1] == 'r' && source_a[2] == 'b' && (source_a[3] == '_' || source_a[3] == '-'))
		{
			...
			if (!error)
			{
				*this = (number_l >> 40).convert_to<raze::uint256_t> ();
...
raze::uint256_union::uint256_union (raze::uint256_t const & number_a)
{
	raze::uint256_t number_l (number_a);
...
bool raze::uint256_union::operator== (raze::uint256_union const & other_a) const
{
	return bytes == other_a.bytes;
}

// Construct a uint256_union = AES_ENC_CTR (cleartext, key, iv)
void raze::uint256_union::encrypt (raze::raw_key const & cleartext, raze::raw_key const & key, uint128_union const & iv)
{
...
bool raze::uint256_union::is_zero () const
{
	return qwords[0] == 0 && qwords[1] == 0 && qwords[2] == 0 && qwords[3] == 0;
}

std::string raze::uint256_union::to_string () const
{
	std::string result;
	encode_hex (result);
	return result;
...
raze::uint256_union & raze::uint256_union::operator^= (raze::uint256_union const & other_a)
{
...
raze::uint256_union raze::uint256_union::operator^ (raze::uint256_union const & other_a) const
{
...
raze::uint256_union::uint256_union (std::string const & hex_a)
{
	decode_hex (hex_a);
}

void raze::uint256_union::clear ()
{
	qwords.fill (0);
}

raze::uint256_t raze::uint256_union::number () const
{
	raze::uint256_t result;
	auto shift (0);
...
void raze::uint256_union::encode_hex (std::string & text) const
{
	assert (text.empty ());
...
bool raze::uint256_union::decode_hex (std::string const & text)
{
	auto error (false);
...
void raze::uint256_union::encode_dec (std::string & text) const
{
	assert (text.empty ());
	std::stringstream stream;
...
bool raze::uint256_union::decode_dec (std::string const & text)
{
...
raze::uint256_union::uint256_union (uint64_t value0)
{
	*this = raze::uint256_t (value0);
}
...
bool raze::uint512_union::operator== (raze::uint512_union const & other_a) const
{
	return bytes == other_a.bytes;
}

raze::uint512_union::uint512_union (raze::uint512_t const & number_a)
{
	raze::uint512_t number_l (number_a);
	for (auto i (bytes.rbegin ()), n (bytes.rend ()); i != n; ++i)
	{
		*i = ((number_l)&0xff).convert_to<uint8_t> ();
		number_l >>= 8;
...
void raze::uint512_union::clear ()
{
	bytes.fill (0);
}

raze::uint512_t raze::uint512_union::number () const
{
	raze::uint512_t result;
	auto shift (0);
	for (auto i (bytes.begin ()), n (bytes.end ()); i != n; ++i)
	{
		result <<= shift;
...
void raze::uint512_union::encode_hex (std::string & text) const
{
	assert (text.empty ());
	std::stringstream stream;
...
bool raze::uint512_union::decode_hex (std::string const & text)
{
...
raze::uint512_union & raze::uint512_union::operator^= (raze::uint512_union const & other_a)
{
...
std::string raze::uint512_union::to_string () const
{
	std::string result;
	encode_hex (result);
	return result;
...
raze::raw_key::~raw_key ()
{
	data.clear ();
}

bool raze::raw_key::operator== (raze::raw_key const & other_a) const
{
...
void raze::raw_key::decrypt (raze::uint256_union const & ciphertext, raze::raw_key const & key_a, uint128_union const & iv)
{
...
raze::uint512_union raze::sign_message (raze::raw_key const & private_key, raze::public_key const & public_key, raze::uint256_union const & message)
{
	raze::uint512_union result;
	ed25519_sign (message.bytes.data (), sizeof (message.bytes), private_key.data.bytes.data (), public_key.bytes.data (), result.bytes.data ());
	return result;
}

void raze::deterministic_key (raze::uint256_union const & seed_a, uint32_t index_a, raze::uint256_union & prv_a)
{
	blake2b_state hash;
...
bool raze::validate_message (raze::public_key const & public_key, raze::uint256_union const & message, raze::uint512_union const & signature)
{
	auto result (0 != ed25519_sign_open (message.bytes.data (), sizeof (message.bytes), public_key.bytes.data (), signature.bytes.data ()));
	return result;
}

raze::uint128_union::uint128_union (std::string const & string_a)
{
	decode_hex (string_a);
}

raze::uint128_union::uint128_union (uint64_t value_a)
{
	*this = raze::uint128_t (value_a);
}

raze::uint128_union::uint128_union (raze::uint128_t const & value_a)
{
	raze::uint128_t number_l (value_a);
	for (auto i (bytes.rbegin ()), n (bytes.rend ()); i != n; ++i)
	{
		*i = ((number_l)&0xff).convert_to<uint8_t> ();
		number_l >>= 8;
...
bool raze::uint128_union::operator== (raze::uint128_union const & other_a) const
{
...
raze::uint128_t raze::uint128_union::number () const
{
...
void raze::uint128_union::encode_hex (std::string & text) const
{
	assert (text.empty ());
	std::stringstream stream;
...
bool raze::uint128_union::decode_hex (std::string const & text)
{
...
void raze::uint128_union::encode_dec (std::string & text) const
{
	assert (text.empty ());
	std::stringstream stream;
	stream << std::dec << std::noshowbase;
...
bool raze::uint128_union::decode_dec (std::string const & text)
{
...
void format_frac (std::ostringstream & stream, raze::uint128_t value, raze::uint128_t scale, int precision)
{
	auto reduce = scale;
	auto rem = value;
...
void format_dec (std::ostringstream & stream, raze::uint128_t value, char group_sep, const std::string & groupings)
{
	auto largestPow10 = raze::uint256_t (1);
	int dec_count = 1;
	while (1)
	{
	...
	auto reduce = raze::uint128_t (largestPow10);
	raze::uint128_t rem = value;
	while (reduce > 0)
	{
		auto val = rem / reduce;
		rem -= val * reduce;
...
std::string format_balance (raze::uint128_t balance, raze::uint128_t scale, int precision, bool group_digits, char thousands_sep, char decimal_point, std::string & grouping)
{
...
std::string raze::uint128_union::format_balance (raze::uint128_t scale, int precision, bool group_digits)
{
...
std::string raze::uint128_union::format_balance (raze::uint128_t scale, int precision, bool group_digits, const std::locale & locale)
{
...
void raze::uint128_union::clear ()
{
	qwords.fill (0);
}

bool raze::uint128_union::is_zero () const
{
	return qwords[0] == 0 && qwords[1] == 0;
}

std::string raze::uint128_union::to_string () const
{
	std::string result;
	encode_hex (result);
	return result;
...
std::string raze::uint128_union::to_string_dec () const
{
	std::string result;
	encode_dec (result);
	return result;
