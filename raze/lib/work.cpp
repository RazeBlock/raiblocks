...
bool raze::work_validate (raze::block_hash const & root_a, uint64_t work_a)
{
	return raze::work_value (root_a, work_a) < raze::work_pool::publish_threshold;
}

bool raze::work_validate (raze::block const & block_a)
{
	return work_validate (block_a.root (), block_a.block_work ());
}

uint64_t raze::work_value (raze::block_hash const & root_a, uint64_t work_a)
{
	uint64_t result;
	blake2b_state hash;
...
raze::work_pool::work_pool (unsigned max_threads_a, std::function<boost::optional<uint64_t> (raze::uint256_union const &)> opencl_a) :
ticket (0),
done (false),
opencl (opencl_a)
{
	static_assert (ATOMIC_INT_LOCK_FREE == 2, "Atomic int needed");
	auto count (raze::raze_network == raze::raze_networks::raze_test_network ? 1 : std::max (1u, std::min (max_threads_a, std::thread::hardware_concurrency ())));
	for (auto i (0); i < count; ++i)
	{
		auto thread (std::thread ([this, i]() {
			raze::work_thread_reprioritize ();
			loop (i);
		}));
...
void raze::work_pool::loop (uint64_t thread)
{
	// Quick RNG for work attempts.
	xorshift1024star rng;
	raze::random_pool.GenerateBlock (reinterpret_cast<uint8_t *> (rng.s.data ()), rng.s.size () * sizeof (decltype (rng.s)::value_type));
	uint64_t work;
	uint64_t output;
	blake2b_state hash;
	...
	while (!done || !pending.empty ())
	{
		...
		if (!empty)
		{
			...
			while (ticket == ticket_l && output < raze::work_pool::publish_threshold)
			{
				...
				while (iteration && output < raze::work_pool::publish_threshold)
				{
					work = rng.next ();
			...
			if (ticket == ticket_l)
			{
				...
				assert (output >= raze::work_pool::publish_threshold);
...
void raze::work_pool::cancel (raze::uint256_union const & root_a)
{
...
void raze::work_pool::generate (raze::uint256_union const & root_a, std::function<void(boost::optional<uint64_t> const &)> callback_a)
{
...
uint64_t raze::work_pool::generate (raze::uint256_union const & hash_a)
{
